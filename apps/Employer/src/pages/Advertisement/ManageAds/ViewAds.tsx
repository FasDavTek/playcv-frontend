...
import { Button, DatePicker, FileUpload, Input, RichTextEditor, HtmlContent } from '@video-cv/ui-components';
...
const ViewAds = () => {
    ...
    useEffect(() => {
        fetchAdDetails();
        fetchAdTypes();
        setIsEditing(isEditingFromState);
    }, [isEditingFromState]);
    ...
    const normalizedAdType = ads?.adType?.toLowerCase() || '';
    ...
    const SubmitForm = async (data: AdFormData) => {
        try {
            ...
            if (media) {
                ...
                // Validate file types
                const isValidFileType = mediaArray.every((file) => {
                    if (!(file instanceof File)) return true;
                    if (normalizedAdType === 'image') return file.type.startsWith('image/');
                    if (normalizedAdType === 'video') return file.type.startsWith('video/');
                    return false;
                });
        
                if (!isValidFileType) {
                    toast.error(`Invalid file type. Please upload only ${normalizedAdType} files.`);
                    return;
                }
