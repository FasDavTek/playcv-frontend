...
const index: React.FC<VideosProps> = ({ page = 1, limit = 10, status, download = false, type = "category", }) => {
  ...
  return (
    <div className='p-4 mb-8'>
      <div className="bg-gray-300 border-b border-gray-200 rounded-lg">
        <div className="flex p-1">
          {['active', 'pending'].map((tab) => (
            ...
            >
              {tab === 'active' ? 'All CVs' : 'Pending CVs'}
            </button>
          ))}
        </div>
      </div>
      ...
      <Table loading={loading} columns={columns} data={videos} search={setSearch} filter={filter} filterConfig={videoFilterConfig} onFilterChange={handleFilterChange} globalFilter={globalFilter} setGlobalFilter={setGlobalFilter} totalRecords={totalRecords} currentPage={currentPage} pageSize={pageSize} onPageChange={handlePageChange} onDownloadCSV={handleDownloadCSV} />
